...
const CAUSES = [
  ...
  { id: 'duplicate_code', label: 'Duplicate catalog code', short: 'Duplicate code', tone: 'orange',
    description: 'The normalized code belongs to catalog records with different subject names or types, and the routine context cannot pick one safely.' },
  { id: 'shared_code', label: 'Shared code · record unresolved', short: 'Shared code', tone: 'blue',
    description: 'The code matches after removing separators, and all matching catalog records share the subject name. Their record IDs differ, so one ID cannot be assigned safely.' },
...
function normalize(records) {
  return records.map((record, index) => {
    // Older exports have two code fields. Present one code in the viewer.
    const { subject_code_raw, ...rest } = record;
    return { ...rest, subject_code: record.subject_code || subject_code_raw || null,
      _id: String(index), _category: categoryOf(record) };
  });
}
...
function Detail({ record }) {
  ...
  return <aside className="detail" aria-label="Case details">
    ...
    <section className="detail-section">
      ...
      <div className="data-line"><span>Subject code</span><strong className="mono">{display(record.subject_code)}</strong></div>
...
export default function App() {
  ...
  const filtered = useMemo(() => records.filter((record) => {
    ...
    const haystack = [record.cell_ref, record.subject_raw, record.subject_name,
